...
// File contracts/Alphazero.sol
// License-Identifier: MIT

pragma solidity 0.6.12;

contract Alphazero is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the ALPHAZERO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ALPHAZERO", "ALPHAZERO") {
        // Mints 1 ALPHAZERO to contract creator for initial pool setup
    ...
    function _getAlphazeroPrice() internal view returns (uint256 _alphazeroPrice) {
        try IOracle(alphazeroOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Alphazero: failed to fetch ALPHAZERO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _alphazeroPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_alphazeroPrice >= taxTiersTwaps[tierId]) {
    ...
    function setAlphazeroOracle(address _alphazeroOracle) public onlyOperatorOrTaxOffice {
        require(_alphazeroOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ALPHAZERO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ALPHAZERO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAlphazeroPrice = _getAlphazeroPrice();
            currentTaxRate = _updateTaxRate(currentAlphazeroPrice);
            if (currentAlphazeroPrice < burnThreshold) {
                burnTax = true;
            }
        }
